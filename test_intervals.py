...
def test_intervaldict_get_set():
    ...
    d = I.IntervalDict([(I.closed(0, 2), 0)])
    assert d.setdefault(I.closed(-2, -1), -1).items() == [(I.closed(-2, -1), -1)]
    assert d.items() == [(I.closed(-2, -1), -1), (I.closed(0, 2), 0)]

    d = I.IntervalDict([(I.closed(0, 2), 0)])
    assert d.setdefault(I.closed(-1, 1), 2).items() == [(I.closedopen(-1, 0), 2), (I.closed(0, 1), 0)]
...
def test_intervaldict_other_methods():
    ...
    # pop intervals
    d = I.IntervalDict([(I.closed(0, 3), 0)])
    assert d.pop(I.closed(0, 1)).items() == [(I.closed(0, 1), 0)]
    assert d.items() == [(I.openclosed(1, 3), 0)]
    assert d.pop(I.closed(0, 2), 1).items() == [(I.closed(0, 1), 1), (I.openclosed(1, 2), 0)]
    assert d.items() == [(I.openclosed(2, 3), 0)]

    # popitem
    d = I.IntervalDict([(I.closed(0, 3), 0)])
    assert d.popitem().items() == [(I.closed(0, 3), 0)]
    assert len(d) == 0
