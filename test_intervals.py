...
def test_intervaldict_get_set():
    ...
    assert d[I.open(-I.inf, I.inf)].items() == set([(I.closed(0, 2), 0)])
    assert d[I.closed(0, 2)].items() == set([(I.closed(0, 2), 0)])
    assert d[I.closed(-1, 0)].items() == set([(I.singleton(0), 0)])
    assert d[I.closed(-2, -1)].items() == set([])
    assert d.get(I.closed(0, 2)).items() == set([(I.closed(0, 2), 0)])
    assert d.get(I.closed(-2, -1)).items() == set([(I.closed(-2, -1), None)])
    assert d.get(I.closed(-1, 0)).items() == set([(I.closedopen(-1, 0), None), (I.singleton(0), 0)])

    d[I.closed(1, 3)] = 1
    assert d.items() == set([(I.closedopen(0, 1), 0), (I.closed(1, 3), 1)])
    assert len(d) == 2
    assert d[0] == 0
    assert d.get(0, -1) == 0
    assert d[1] == 1
    ...
    d[I.closed(0, 2)] = 1
    assert d.items() == set([(I.closed(0, 2), 1), (I.singleton(3), 3)])
    d[I.closed(-1, 1)] = 2
    assert d.items() == set([(I.closed(-1, 1), 2), (I.openclosed(1, 2), 1), (I.singleton(3), 3)])

    d = I.IntervalDict([(I.closed(0, 2), 0)])
    d[I.closed(-1, 4)] = 1
    assert d.items() == set([(I.closed(-1, 4), 1)])
    d[I.closed(5, 6)] = 1
    assert d.items() == set([(I.closed(-1, 4) | I.closed(5, 6), 1)])
    ...
    assert t.items() == set([(I.closed(-2, -1), -1)])
    assert d.items() == set([(I.closed(-2, -1), -1), (I.closed(0, 2), 0)])

    d = I.IntervalDict([(I.closed(0, 2), 0)])
    t = d.setdefault(I.closed(-1, 1), 2)
    assert t.items() == set([(I.closedopen(-1, 0), 2), (I.closed(0, 1), 0)])
    assert d.items() == set([(I.closedopen(-1, 0), 2), (I.closed(0, 2), 0)])


def test_intervaldict_iterators():
    d = I.IntervalDict([(I.closedopen(0, 1), 0), (I.closedopen(1, 3), 1), (I.singleton(3), 2)])

    assert d.keys() == set([I.closedopen(0, 1), I.closedopen(1, 3), I.singleton(3)])
    assert d.domain() == I.closed(0, 3)
    assert d.values() == set([0, 1, 2])
    assert d.items() == set(zip(d.keys(), d.values()))
    assert set(d) == d.keys()

    # Iterators on empty
    assert I.IntervalDict().values() == set()
    assert I.IntervalDict().items() == set()
    assert I.IntervalDict().keys() == set()
...
def test_intervaldict_other_methods():
    ...
    assert a == b
    assert a != 1
    assert a.items() == set([(I.closed(-1, 1), 2), (I.openclosed(1, 2), 0), (I.closed(4, 5), 1)])
